...
| better-sqlite3 | 11.x | SQLite 嵌入式数据库 |
| cors | 2.x | 跨域处理 |
...
│   ├── 数据库文件路径：/data/cms.db（Docker volume 持久化）
...
├── mockData.ts       # 初始 Mock 数据（为各页面提供演示数据）
├── appReducer.ts     # 统一 Reducer（全部 Action 处理）
└── appContext.tsx    # AppProvider + useAppStore() hook
                      # 持久化策略：localStorage（同步）→ SQLite REST API（异步双写）→ Mock 数据（兜底）
```

**数据持久化优先级**：
1. 启动时：优先从 SQLite（db-server）读取 → 失败则读 localStorage → 再失败则用 mockData
2. 运行时：每次状态变更同步写 localStorage + 异步 upsert 到 SQLite
3. 删除操作：同时从 localStorage 和 SQLite 中删除（通过 id diff 检测）
...
- [x] **SQLite 持久化（db-server）**：完整 CRUD REST API，appContext 三级降级 + 双写策略，删除操作同步 SQLite
