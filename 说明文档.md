...
### P2/P1 Patch：同步 pnpm-lock.yaml 中 undici 依赖（2026-05-01 完成 ✅）

**目标**：同步 pnpm-lock.yaml 中 undici 依赖，确保 Docker 构建可复现。
**变更内容**：
1. **依赖同步**：通过 `pnpm install --lockfile-only --no-frozen-lockfile` 刷新并同步了 `pnpm-lock.yaml`，使其 undici 依赖定义与 `package.json` 和 `package-lock.json` 版本严格一致。
2. **构建确定性收口**：不修改业务代码或包版本，单纯收口依赖解析差异，提升了以后 CI 流程、缓存复用、frozen install 及机器切换部署环境下的构建确定性和稳定性。

**涉及文件**：
- `pnpm-lock.yaml`
- `说明文档.md`

### P1 Patch：AI Metadata v0.2 方案结构对齐、Evidence 归一与兼容投影修正 (2026-05-01 完成 ✅)
- [x] 1. 方案文档纳入仓库
- [x] 2. 明确 Luceon canonical v0.2 schema
- [x] 3. system_tags 最小系统生成
- [x] 4. governance_signals 最小结构
- [x] 5. Evidence 归一
- [x] 6. 兼容投影修正
- [x] 7. 最小受控值校验
- [x] 8. 前端详情页兼容展示
