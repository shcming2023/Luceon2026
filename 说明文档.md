...
### 阶段二十七：material 重复记录修复（已完成 ✅, 2026-04-22）

- [x] **material 重复记录修复**：`POST /tasks` 创建 ParseTask 时优先复用前端传入的 `materialId`，避免同一文件在解析过程中产生多条冗余的 material 记录。
- [x] **文档同步**：同步更新说明文档，记录阶段二十七里程碑。
