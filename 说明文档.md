...
### 阶段三十八：AI 终态语义收敛与运维安全加固 (待执行 ⏳, 2026-04-23)

- [ ] **Secrets 契约正式同步**：将 `db-server.mjs` 中已修复的 `/secrets` 字段校验逻辑（兼容新旧 key）正式提交 GitHub，并同步到 `types.ts`（若有需要）。
- [ ] **AI 审核状态判定修正**：重构 `AiMetadataWorker` 的状态判定逻辑。若 `result.needsReview === true`，ParseTask 必须进入 `review-pending` 状态，即使置信度达标，确保人工介入语义正确。
- [ ] **状态映射规则收敛**：明确并修正 `Material.status` 与 `Task.state` 的映射关系。特别是 `review-pending` 状态下 Material 的展示状态，避免混淆。
- [ ] **一致性清理安全增强**：在 `consistency-routes.mjs` 中增加 `dry-run` 模式支持。新增 UI 或脚本入口（如 `/audit/apply-safe`），提供“预览 -> 确认 -> 执行”的清理流程，防止历史数据误删。
- [ ] **双链路 E2E 强化**：补充 PDF 与 Markdown 独立上传链路的 UAT 测试用例，断言任务、资料、AI Job 三方状态的一致性收敛。
