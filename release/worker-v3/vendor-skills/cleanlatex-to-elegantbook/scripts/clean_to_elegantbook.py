...
def repair_rowbreaks_inside_text_commands(text: str) -> str:
    ...
    text_body = r"(?:\\[{}]|[^{}\n])*?"
    pattern = re.compile(rf"\\text\s*\{{(?P<body>{text_body})\s*\\\\\s*")
...
def wrap_cjk_text_in_math(value: str) -> str:
    ...
    value = re.sub(r"\\text\s*\{[^{}\n]*\}", protect, value)
    value = re.sub(r"([\u3400-\u9fff]+)", r"\\text{\1}", value)
    for idx, original in enumerate(placeholders):
...
def build_elegantbook_main(
    ...
    cover_line = f"\\cover{{{cover_image}}}\n" if cover_image else ""
    logo_line = f"\\logo{{{logo_image}}}\n" if logo_image else ""
    plain_cover_override = "" if cover_image else plain_text_cover_override(
        title,
        subtitle,
        author,
        date_text,
    ...
    return f"""{package_option_prelude(demo_images)}\\documentclass[lang=cn,11pt,openany]{{elegantbook}}
\\title{{{title}}}
\\subtitle{{{subtitle}}}
\\author{{{author}}}
\\date{{{date_text}}}
\\version{{{version}}}
\\extrainfo{{{extrainfo}}}
{cover_line}{logo_line}
{plain_cover_override}
...
\\fancyfoot[c]{{\\color{{structurecolor}}\\small\\arabic{{page}}}}
\\fancypagestyle{{plain}}{{\\renewcommand{{\\headrulewidth}}{{0pt}}\\fancyhf{{}}\\fancyfoot[c]{{\\color{{structurecolor}}\\small\\arabic{{page}}}}}}
...
def write_project(args: argparse.Namespace) -> None:
    ...
    content = transform_content(source)
    content = normalize_image_includes(content, input_path, out_dir)
    content = final_tex_safety_pass(content)
    cover_image = copy_optional_asset(args.cover_image, input_path, out_dir)
    logo_image = copy_optional_asset(args.logo_image, input_path, out_dir)
    ...
    if cover_image:
        notes.append(f"Cover image: {cover_image}")
    if logo_image:
        notes.append(f"Logo image: {logo_image}")
    if missing:
        notes.append("")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--date", default=r"\today", help="Date text for the title page")
