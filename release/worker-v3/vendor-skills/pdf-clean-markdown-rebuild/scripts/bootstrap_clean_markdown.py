...
try:
    from build_outline_decision import build_decision, maybe_review_with_llm
except Exception:
    build_decision = None
...
SPACED_DIGIT_RUN_RE = re.compile(r"(?<![A-Za-z\\])\d(?:[ \t]+\d)+(?![A-Za-z])")
...
def page_is_chapter_opener(page_blocks):
    ...
    if not headings:
        return False
    page_texts = [block_text(block) for block in page_blocks if block_text(block)]
    has_number = any(re.fullmatch(r"\d{1,2}", text) for text in headings + page_texts)
    has_title = any(
    ...
    return has_number and has_title and (has_learning_signal or len(headings) <= 3)
...
def infer_body_range(blocks):
    ...
    for page in sorted_pages:
        if page <= start_page:
            continue
        if any(is_back_matter_text(text) for text in pages[page]):
            end_page = page - 1
            break
...
def markdown_image_refs(markdown):
    refs = set()
    for md_ref in re.findall(r"!\[[^\]]*\]\(([^)]+)\)", markdown):
...
def main():
    ...
    inferred_start, inferred_end, first_body_text_page, last_front_page = infer_body_range(blocks)
    ...
    if args.scope == "body" and popo_outline.get("available"):
        ...
        if args.start_page is None and isinstance(popo_start, int):
            start_page = max(0, popo_start)
            first_body_text_page = start_page
        if args.end_page is None and isinstance(popo_end, int):
            end_page = popo_end
    ...
    for block in blocks:
        ...
        if page != last_page:
            ...
            if args.scope == "body" and not popo_outline.get("available") and page_canonical_chapter and page_canonical_chapter != current_chapter:
                lines.extend(["", f"# {page_canonical_chapter}"])
                current_chapter = page_canonical_chapter
            if args.scope == "body" and not popo_outline.get("available") and first_unit and not injected_initial_unit:
            ...
            if args.scope == "body" and not popo_outline.get("available") and page_unit and page_unit != current_unit:
        ...
        if btype == "page_footnote":
            text = clean_text(block.get("text"))
            if text:
                footnotes_by_page.setdefault(page, []).append(text)
            continue
            ...
            if body:
                ...
                if img_rel:
                    if copy_images and (root / img_rel).exists():
                        ...
                        missing.add(img_rel)
                    lines.extend(["", f"![{caption or 'table'}]({img_rel})"])
            continue
    ...
    if footnotes_by_page:
        lines.extend(["", "<!-- Footnotes -->"])
        for page in sorted(footnotes_by_page, key=lambda x: (-1 if x is None else x)):
            lines.extend(["", f"Page {page} footnotes:"])
            for note in footnotes_by_page[page]:
                lines.append(f"- {note}")
    ...
    output_popo_outline = dict(popo_outline)
    ...
    report_lines = [
        ...
        f"- Popo outline available: `{bool(popo_outline.get('available'))}`; outline entries: `{len(popo_outline.get('outline') or [])}`",
