...
// AI Metadata Worker with onComplete callback to sync results back to materials table
const aiWorker = new AiMetadataWorker({
  ...
  onComplete: async (job, update) => {
    try {
      ...
      const isSuccess = update.state === 'confirmed' || update.state === 'review-pending';
      const result = update.result || {};
      const patchBody = {
        aiStatus: isSuccess ? 'analyzed' : 'failed',
        tags: result.tags || [],
        metadata: {
          aiConfidence: result.confidence,
          aiNeedsReview: update.state === 'review-pending',
          aiProviderId: job.providerId,
          aiModel: job.model,
          processingMsg: isSuccess
            ? `AI 元数据识别完成 (置信度: ${result.confidence || 0}%)`
            : `AI 元数据识别失败: ${update.errorMessage || '未知错误'}`
        }
      };

      const dbBaseUrl = process.env.DB_BASE_URL || 'http://localhost:8789';
      const resp = await fetch(`${dbBaseUrl}/materials/${materialId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!resp.ok) {
        console.error(`[upload-server] Failed to sync AI result to materials: ${resp.status} ${resp.statusText}`);
      } else {
        console.log(`[upload-server] Synced AI result to materials/${materialId}: ${patchBody.aiStatus}`);
      }
    } catch (err) {
      console.error('[upload-server] Error in onComplete callback:', err.message);
    }
  }
});
