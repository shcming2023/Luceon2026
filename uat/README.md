...
# 2. 确认 .env 已配置（CMS_PORT=8080 已设置）
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `CMS_PORT` | `8080` | 对外暴露端口 |
| `CMS_HOST` | `localhost` | 对外展示的主机名（启动信息中使用） |
| `MINIO_ENDPOINT` | `localhost` | MinIO 主机（容器内可达地址） |
| `MINIO_PUBLIC_ENDPOINT` | — | presigned URL 公开地址（如 `http://YOUR_HOST:8080/minio`） |
...
# 或指定目标地址
BASE_URL=http://YOUR_HOST:8080 ./uat/smoke-test.sh
```
...
# 运行所有测试（默认 localhost:8080）
npx playwright test

# 指定目标地址和公网主机名
BASE_URL=http://YOUR_HOST:8080 PUBLIC_HOST=YOUR_HOST npx playwright test
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `BASE_URL` | `http://localhost:8080` | 测试目标地址 |
...
- [ ] 浏览器打开 `http://YOUR_HOST:8080`，自动跳转到 `/cms/source-materials`
- [ ] 页面正常渲染，侧边栏导航可切换
- [ ] 在「原始资料库」页面上传一个 PDF/图片文件
- [ ] 上传成功后文件可在列表中显示，预览可正常加载
- [ ] 检查文件 URL 格式为 `http://YOUR_HOST:8080/minio/...`（非 `minio:9000`）
- [ ] 添加/修改数据后刷新页面，数据保持不变
...
上传文件
  ...
  → rewritePresignedUrl() 替换为:
             http://YOUR_HOST:8080/minio/eduassets/originals/xxx.pdf?X-Amz-...
  → 浏览器 GET http://YOUR_HOST:8080/minio/...
  → 代理转发到 MinIO:9000
  → 文件正常加载 ✓
```
...
# 执行基础链路 Bash 冒烟
BASE_URL=http://127.0.0.1:8080 bash uat/smoke-test.sh
```

### 2. E2E 链路与一致性复验
```bash
# 执行完整 Pipeline 一致性 E2E（Playwright）
cd uat && BASE_URL=http://127.0.0.1:8080 npx playwright test tests/pipeline-consistency.spec.ts

# 执行页面可用性回归（防止 React 运行时崩溃）
cd uat && BASE_URL=http://127.0.0.1:8080 npx playwright test tests/pages-smoke.spec.ts

# 查看数据一致性审计结果 (Dry-run)
curl -sS http://127.0.0.1:8080/__proxy/upload/audit/consistency
```
...
### 1. 明确的命令兼容性
- **Windows 宿主机**：请使用 `npm.cmd` 和 `npx.cmd` 代替 `npm` 和 `npx`。
- **macOS / Linux / WSL / Bash**：请继续使用 `npm` 和 `npx`。

### 2. 当前端口基线
- CMS Frontend：`8080`
- MinerU (本地映射)：`8083` -> `8080`
- Ollama：`11434`
- MinIO Console：`19001`
